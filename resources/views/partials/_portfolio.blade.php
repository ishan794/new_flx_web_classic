<section id="portfolio" class="py-16 lg:py-24 bg-navy-dark">
    <div class="container max-w-7xl mx-auto px-6">
        
        <div class="text-center mb-16 scroll-animate">
            <span class="text-accent text-sm tracking-widest uppercase font-semibold block mb-4">OUR WORK</span>
            <h2 class="text-3xl lg:text-4xl font-bold text-white mb-4">Our Portfolio</h2>
            <p class="text-gray-400 max-w-2xl mx-auto text-base leading-relaxed">
                Explore our recent projects and see how we've helped businesses transform their ideas into successful digital solutions.
            </p>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            
            <!-- Project 1 -->
            <div class="bg-navy-mid border border-white/10 rounded-2xl p-8 hover:border-accent/40 hover:-translate-y-1 transition-all duration-300 scroll-animate group flex flex-col h-full">
                <div class="flex justify-between items-start mb-8">
                    <div class="w-12 h-12 bg-accent/10 rounded-xl flex items-center justify-center text-accent transition-transform group-hover:scale-110 duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>
                    </div>
                    <div class="text-right">
                        <span class="text-white text-sm font-medium block">Mobile App</span>
                        <span class="text-gray-500 text-xs">2024</span>
                    </div>
                </div>
                
                <h3 class="text-white font-bold text-2xl mb-3 group-hover:text-accent transition-colors">Expense Tracker</h3>
                <p class="text-gray-400 text-sm leading-relaxed mb-8 flex-grow">
                    A comprehensive personal finance management app that helps users track expenses, set budgets, and achieve financial goals.
                </p>
                
                <ul class="text-gray-400 text-sm space-y-3 mb-8">
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Expense Categorization</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Budget Planning</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Financial Reports</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Goal Tracking</span>
                    </li>
                </ul>
                
                <div class="pt-6 border-t border-white/5 mb-6">
                    <div class="flex flex-wrap gap-3 text-xs text-gray-500 font-medium">
                        <span>Flutter</span>
                        <span>&bull;</span>
                        <span>Node.js</span>
                        <span>&bull;</span>
                        <span>Firebase</span>
                    </div>
                </div>
                
                <a href="#" class="inline-flex items-center text-sm font-semibold text-white group-hover:text-accent transition-colors mt-auto">
                    View Project <span class="ml-2 group-hover:translate-x-1 transition-transform">&rarr;</span>
                </a>
            </div>

            <!-- Project 2 -->
            <div class="bg-navy-mid border border-white/10 rounded-2xl p-8 hover:border-accent/40 hover:-translate-y-1 transition-all duration-300 scroll-animate group flex flex-col h-full" style="transition-delay: 100ms;">
                <div class="flex justify-between items-start mb-8">
                    <div class="w-12 h-12 bg-accent/10 rounded-xl flex items-center justify-center text-accent transition-transform group-hover:scale-110 duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m18 15-6-6-6 6"/></svg>
                    </div>
                    <div class="text-right">
                        <span class="text-white text-sm font-medium block">Mobile App</span>
                        <span class="text-gray-500 text-xs">2024</span>
                    </div>
                </div>
                
                <h3 class="text-white font-bold text-2xl mb-3 group-hover:text-accent transition-colors">Exercise Tracker</h3>
                <p class="text-gray-400 text-sm leading-relaxed mb-8 flex-grow">
                    A fitness tracking application that monitors workouts, tracks progress, and provides personalized fitness recommendations.
                </p>
                
                <ul class="text-gray-400 text-sm space-y-3 mb-8">
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Workout Logging</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Progress Analytics</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Custom Routines</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Personalized Recommendations</span>
                    </li>
                </ul>
                
                <div class="pt-6 border-t border-white/5 mb-6">
                    <div class="flex flex-wrap gap-3 text-xs text-gray-500 font-medium">
                        <span>React Native</span>
                        <span>&bull;</span>
                        <span>Firebase</span>
                        <span>&bull;</span>
                        <span>Dart</span>
                    </div>
                </div>
                
                <a href="#" class="inline-flex items-center text-sm font-semibold text-white group-hover:text-accent transition-colors mt-auto">
                    View Project <span class="ml-2 group-hover:translate-x-1 transition-transform">&rarr;</span>
                </a>
            </div>

            <!-- Project 3 -->
            <div class="bg-navy-mid border border-white/10 rounded-2xl p-8 hover:border-accent/40 hover:-translate-y-1 transition-all duration-300 scroll-animate group flex flex-col h-full" style="transition-delay: 200ms;">
                <div class="flex justify-between items-start mb-8">
                    <div class="w-12 h-12 bg-accent/10 rounded-xl flex items-center justify-center text-accent transition-transform group-hover:scale-110 duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
                    </div>
                    <div class="text-right">
                        <span class="text-white text-sm font-medium block">Web Platform</span>
                        <span class="text-gray-500 text-xs">2025</span>
                    </div>
                </div>
                
                <h3 class="text-white font-bold text-2xl mb-3 group-hover:text-accent transition-colors">Wayz</h3>
                <p class="text-gray-400 text-sm leading-relaxed mb-8 flex-grow">
                    A vehicle rental platform designed specifically for tourists, offering seamless booking and local transportation solutions.
                </p>
                
                <ul class="text-gray-400 text-sm space-y-3 mb-8">
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Vehicle Booking</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Tourist Guide Integration</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>Payment Processing</span>
                    </li>
                    <li class="flex items-start">
                        <span class="text-accent mt-0.5 mr-3"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span> 
                        <span>GPS Navigation</span>
                    </li>
                </ul>
                
                <div class="pt-6 border-t border-white/5 mb-6">
                    <div class="flex flex-wrap gap-3 text-xs text-gray-500 font-medium">
                        <span>Flutter</span>
                        <span>&bull;</span>
                        <span>NodeJS</span>
                        <span>&bull;</span>
                        <span>AWS</span>
                    </div>
                </div>
                
                <a href="#" class="inline-flex items-center text-sm font-semibold text-white group-hover:text-accent transition-colors mt-auto">
                    View Project <span class="ml-2 group-hover:translate-x-1 transition-transform">&rarr;</span>
                </a>
            </div>

        </div>
    </div>
</section>
